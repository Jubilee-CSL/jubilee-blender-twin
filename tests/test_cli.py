...
def _latest_gcode() -> str:
    from jubilee_twin.paths import resolve
    gcode = resolve("gcode_logs_dir") / "latest.gcode"
    if not gcode.exists():
...
def test_prepare_creates_pipeline_data(tmp_path):
    ...
    try:
        ...
        state_path = str(machine_state_json())
    except ImportError:
        pytest.skip("science_jubilee not installed")
    import os
    if not os.path.isfile(state_path):
        pytest.skip("pipeline_data/machine_state.json not found — run an experiment first")
